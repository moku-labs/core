...
| 8 | Annotate `register` when `api` is a pre-typed factory | Unannotated `register => ...` is context-sensitive, so TypeScript defers it and checks `api` first with an empty own-event map. Either `events: (register: RegisterFunction) => ...` moves the callback into the first pass, or `api: ctx => createApi(ctx)` moves the factory into the second pass. |
