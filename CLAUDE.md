...
**@moku-labs/core** is a micro-kernel plugin framework for TypeScript. Two runtime exports (`createCoreConfig`, `createCorePlugin`), three layers (core -> framework -> consumer), each constraining the layer above. The entire API fits in an LLM context window by design.

**Status:** Released — `0.1.x` on npm as `@moku-labs/core`. The public API (`createCoreConfig` -> `createCore` -> `createApp`) is stable; pre-1.0 means internals may still be refined.

## Architecture: Three Layers

- **Layer 1 (@moku-labs/core):** Two runtime exports: `createCoreConfig` (Step 1 of the factory chain) and `createCorePlugin` (self-contained infrastructure plugins). Zero domain knowledge. Pure machinery: lifecycle, plugin registry, event bus, config resolution, type inference.
...
- **Layer 3 (Consumer):** Imports from the framework. Single call with structured namespaces: `createApp({ plugins?, config?, pluginConfigs?, onReady?, onError?, onStart?, onStop? })`. Never sees `@moku-labs/core` directly.
...
| `specification/02-CORE-API.md` | All function signatures, createCoreConfig/createCore/createApp |
...
| `specification/15-PLUGIN-STRUCTURE.md` | Plugin file organization |
...
- Keep the runtime minimal. The type system does the heavy lifting.
- Bundle: < 8KB gzipped, zero runtime dependencies.
